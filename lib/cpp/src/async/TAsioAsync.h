...
class TAsioClient : public boost::enable_shared_from_this<TAsioClient> {
 ...
 public:
  ...
  template<typename T>
    void connect(const std::string& host, uint16_t port, void (&callback)(boost::shared_ptr<T> client)) {
      ...
      connectF(host, port, callback2);
    }

  template<typename T>
    void connectF(const std::string& host, uint16_t port, boost::function<void (boost::shared_ptr<T> client)> callback) {
 ...
 private:
  ...
  void connectSimple(const std::string& host, uint16_t port, boost::function<void (void)> callback);
